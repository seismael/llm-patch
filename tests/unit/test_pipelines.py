...
from llm_patch.core.models import AdapterManifest, DocumentContext, ModelHandle
...
class TestCompilePipeline:
    def _make_deps(self):
        ...
        repo.save.side_effect = lambda aid, w, cfg: AdapterManifest(
            adapter_id=aid, rank=8, target_modules=["q_proj"], storage_uri=f"/tmp/{aid}"
        )
    ...
    def test_context_manager(self):
        ...
        with CompilePipeline(source, gen, repo, stream=stream) as pipeline:
            stream.start.assert_called_once()
