...
class TestPeftAgentRuntime:
    ...
    def test_generate_returns_decoded_text(self, _mock_torch):
        ...
        handle.tokenizer.return_value.__getitem__ = lambda _self, key: input_dict.get(
            key, MagicMock()
        )
        ...
        output_ids.__getitem__ = lambda _self, _idx: MagicMock()
    ...
    @patch("llm_patch.runtime.agent.torch", create=True)
    def test_chat_returns_chat_response(self, _mock_torch):
        from llm_patch.runtime.agent import PeftAgentRuntime
...
class TestChatSession:
    def _make_runtime(self):
        ...
        runtime.chat = MagicMock(
            return_value=ChatResponse(message=ChatMessage(role=ChatRole.ASSISTANT, content="reply"))
        )
        return runtime
