...
def _make_agent(
    ...
    if agent_type == "litellm":
        try:
            from llm_patch.wiki.agents.litellm_agent import DEFAULT_MODEL as LITELLM_DEFAULT_MODEL
        ...
        return LiteLLMWikiAgent(
            model=model or LITELLM_DEFAULT_MODEL,
            api_key=api_key,
            schema=schema,
        )
    ...
    try:
        from llm_patch.wiki.agents.anthropic_agent import DEFAULT_MODEL as ANTHROPIC_DEFAULT_MODEL
...
def obsidian_init(
    ...
    filters = [".git", "raw", *ignore_filters]
    cfg = ObsidianConfig(
        enabled=True,
