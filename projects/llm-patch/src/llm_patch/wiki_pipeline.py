...
class _CompileAllRunner(Protocol):
    def compile_all(self) -> object:
        """Run a full compile cycle."""


_DEPRECATION_MESSAGE = (
    "WikiPipelineOrchestrator is deprecated and will be removed in a future major "
    "release. Use llm_patch.WikiPipeline instead."
)
...
class WikiPipelineOrchestrator:
    ...
    def __init__(
        ...
        agent: IWikiAgent,
        config: WikiConfig,
        adapter_orchestrator: _CompileAllRunner | None = None,
    ) -> None:
    ...
    def _trigger_adapter_rebuild(self, result: IngestResult) -> None:
        if self._adapter_orchestrator is None:
            return
        ...
        try:
            self._adapter_orchestrator.compile_all()
        except Exception:
