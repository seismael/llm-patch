...
def _resolve_source(source_dir: Path, patterns: list[str], recursive: bool):
...
def _resolve_generator(checkpoint_dir: Path | None, device: str):
    """Create a weight generator — requires a T2L checkpoint."""
    if checkpoint_dir is None:
        raise click.ClickException(
            "Missing --checkpoint-dir. "
            "Provide a T2L hypernetwork checkpoint directory."
        )
...
def _resolve_repository(output_dir: Path):
...
def _build_orchestrator(
    ...
    return KnowledgeFusionOrchestrator(
