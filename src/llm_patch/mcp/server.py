...
try:
    ...
    raise ImportError(
        ...
        "Install with: pip install 'llm-patch[mcp]'"
    )
...
def wiki_guide() -> str:
    ...
    for pt in schema.page_types:
        sections = ", ".join(pt.required_sections) if pt.required_sections else "none"
        lines.append(f"- **{pt.name}**: {pt.description} (sections: {sections})")
    lines.append("")
    lines.append("## Rules")
